...
\chapter{Kerning and Abugidas}

In this lesson we will learn how to finesse the horizontal spacing of two characters.
This can help with the accents of the prior lesson, but is also required for abugidas.

\section{Introduction}

Abugidas come is a few varieties but what makes an abugida is that there is a vowel added to a
base character which changes the implied vowel to something else.
In effect these vowels can almost feel like extra large accents with added complexities.
They can often take some space, or not, depending on the vowel.
They can also require specialized placement, depending on the consonant and vowel being combined.

\section{Language}

Rather than make a brand new language, we are going to extend our last one.
This language will have two consonants: the character that looks like an o and a character that
looks like an \i.
It will have two vowels, the hat above and the curve below.

Since I know that I have consonants and vowels here, I am going to take it one step further and
assume that I know which consonants and vowels I have.
The o character will be the sound of /b/, the \i\ character will be the sound of /d/.
Both will have an implied vowel of a.
The hat above will be the vowel of e, and the curve below will be the vowel of i.
That means that even though I only have two visible vowels, I actually need three.
...
\begin{quote}
\begin{tabular}{lcl}
\texttt{iofont.mf}&---&The entire font.\\
\texttt{io\_b.mf} &---&The /b/ character which looks like an o to us.\\
\texttt{io\_d.mf} &---&The /d/ character which looks like an \i\ to us.\\
\texttt{io\_a.mf} &---&An empty accent for the implied vowel.\\
\texttt{io\_e.mf} &---&The e accent which looks like a hat.\\
\texttt{io\_i.mf} &---&The i accent which sits below.\\
\end{tabular}
\end{quote}

Let's build this up the reverse direction of how we built up our last font.
That's probably not the correct way to do it, but there is a pedantic reason for doing that.

\subsection{\texttt{io\_i.mf}}

This is almost a copy of our low accent.
Rather than have \texttt{-11u} in order to center the accent, we are only moving back \texttt{-7u}
to that the right side of the accent aligns with the 0 width.
...
\subsection{\texttt{io\_e.mf}}

This is almost a copy of our high accent.
It has a similar change as our last character and for the same reason.
...
\subsection{\texttt{io\_a.mf}}

For this accent we want to take no space and want to also not draw anything.
So why did we bother?
Because we want to be able to type things like "ba" and not have \TeX\ complain about an
undefined character.
\TeX\ is just as happy to not paint anything and not move as it is to paint and move.
...
\subsection{\texttt{io\_d.mf}}

This is a brand new character, but should come as no surprise.
It is thinner, but otherwise similar to the next letter.
...
Those uses of the command \texttt{ligtable} are new.
A \texttt{ligtable} is a comma separated list of what to do with pairs of characters.
The first pair is ``BE'' which causes a \texttt{kern} of -0.5u.
So what is a \texttt{kern}?
Kerning is a well established technique to help out when letters look just too far apart.
The most obvious example is A{}V which is kerned to be AV.
In our case we mean that this will add $-0.5u$ between the two letters --- remove $0.5u$ from between the two letters.
So the accent will be placed $0.5u$ left of where it was designed to be instead of at the right-most edge.

So how did I come up with these numbers?
What in the \texttt{dvi} file shows you what these look like?
Absolutely nothing.
In order to come up with these numbers, I had to make a \TeX\ file and test them out.
Here's the file to try it out with after you have created the fonts:
...
\begin{quote}
\begin{tabular}{rl}
1.&Edit \texttt{iofont.mf};\\
2.&Run \verb|mf '\mode=ljfour; mode_setup; input iofont.mf'|;\\
3.&Run \texttt{gftopk iofont.600gf};\\
4.&Run \TeX\ on \texttt{iofonttest.tex}; and\\
5.&View the results.\\
\end{tabular}
\end{quote}

Only after doing all that did I know if other adjustments were needed.
Doing this can be a bit frustrating though.
In order to see how it looked, I was zoomed in and you really start to notice that your
\texttt{pdf} file is showing pictures and not full-fledged letters --- but that's a later lesson.

So everything looks good, let's try adjusting our \texttt{iofonttest.tex} file to see running text
...
What happened!
We adjusted the accents so far to the left that they did not leave enough room for the base
letters.
One possible fix is to make those accents have \emph{some} space instead of a width of 0.
Another possibility is to adjust the width of the base letters so they have space for the
accents.
Neither solution is very good, but $\ldots$

\begin{exercise}
Increase the width of some characters until they don't overlap.

Having a \texttt{iofonttest.tex} file with lines of B followed by lines of Be will help you
find the amount of space you need.
\end{exercise}

\section{Extras}

In our own alphabet, certain letter combinations look like they are too far apart when in a word
together.
Compare:
...
In order to accomplish this there were letters where the edge of the letter stuck out past the
past the base.
So there was a ``W'' that had a full amount of space around it and another ``W'' with the right
end hanging over so that the following ''a'' could be just a little closer.
This overhang was called a \emph{kern} and the process of selecting these extra letters was
called \emph{kerning}.
Having little bits of metal hang off the edge makes for type that has to be re-purchased more
often, but it does make for better looking documents so it is a cost many were willing to pay
for.
Go find some images of books from the 17$^{\rm th}$ or 18$^{\rm th}$ century.
Take a look at some expensive ones like a highly regarded classic and also take a look at some of
the cheaper ones as well.
Now that you know about this, you will probably notice it.
You are less likely to notice it in a modern book because almost all systems do this kerning
automatically now.

One way that \MF\ could have handled this would have been to exactly model the physical process.
You could define a letter for ``W'' with it's full width and then defined a second letter with
the exact same drawing instructions (just like our first alphabet) but with less width.
This would then require the person who is working in \TeX\ to remember that there are two letters.
Then the user of \TeX\ would have to remember to type ``\verb|Wh|'' normally and
``\verb|\kernedW o|'' otherwise (or maybe ``\verb|\fullW h|'' and ``\verb|Wo|'').

Instead, \MF\ has this table which tells \TeX\ ``if you want an W followed by an o then it will
only take this much space, otherwise it takes this much space''.
What \TeX\ effectively sees is two characters but you only have to define the shape once.

So if this is called kerning, why is the command called \texttt{ligtable}?
The command \texttt{ligtable} is short for \emph{ligature table}.

When using physical type there are limits to how much you can kern.
Some letter combinations require so much kerning to look good together that you can't actually
kern them.
In those cases, rather than have one kerned character, you would have a special character that put
both letters together on one piece of lead.
This special ``two-for-one'' piece of type was called a ligature.
They were not as fragile as a kerned letter so they tended to be used even for less expensive
books.
Eventually the letter forms of certain ligatures were shaped differently to fit together even
better.

\begin{center}
\begin{tabular}{ccc}
{\Huge f{}i}&{\Huge f\kern -2pt i}&{\Huge fi}\\
Normal&Kerned&Ligature\\
\end{tabular}
\end{center}

Because of the physical limitations of kerning with lead type, accents would have been handled by
ligatures instead.
But with \MF\ we have no such limitations so we can even kern over the entire letter.

This has almost turned into a preview for the next lesson, instead of just extra material, so
let's move on.

\chapter{Ligatures and Syllabaries}

Ligatures let us use the twenty-six letters of our alphabet to access many more symbols.
If you are making a syllabary, then this is the technique you need.
Our font will consist of ligatures, which requires more glyphs that letters, and some rules that
say ``when these two letters are next to eachother, then use this glyph instead.''
And there is no need to limit yourself to just two.
\TeX\ turns -{}-{}- into --- by building up a character ligature.
So even though our syllabary will consist of very open syllables like \emph{ba} you can make
syllabaries that combine as many letters as you want, assuming you have enough space.

\section{Introduction}

I mentioned in the lesson on alphabets that the difference between alphabets and ideographs was a
question of quantity and not complexity.
I also mentioned that ideographs required creating multiple fonts and switching between the fonts.
But syllabaries exist somewhere between.
There are more symbols in them than we generally have (easily) accessible from a
keyboard.
Ligatures will let you bypass this limitation and gain access of up to two-hundred-fifty-six
unique symbols.
Enough to invent a syllabary which is way too large.

\section{Language}

We are going to revisit our \texttt{iofont} but this time instead of using kerning, we will make
unique glyphs for each of our letters.
We are also going to introduce a new vowel of \emph{o} but not use an accent for it instead
opting for a brand new shape.
Since we can do this, we are also free to adjust the vertical spacing of our accents.
...
\begin{quote}
\begin{tabular}{lclcl}
\texttt{sylfont.mf}   &---&The entire font.\\
\texttt{syl\_ba.mf}   &---&The /ba/ syllable&---&o.\\
\texttt{syl\_be.mf}   &---&The /be/ syllable&---&o with a hat.\\
\texttt{syl\_bi.mf}   &---&The /bi/ syllable&---&o with a chin.\\
\texttt{syl\_bo.mf}   &---&The /bo/ syllable&---&v.\\
\texttt{syl\_da.mf}   &---&The /da/ syllable&---&\i.\\
\texttt{syl\_de.mf}   &---&The /de/ syllable&---&\i\ with a hat.\\
\texttt{syl\_di.mf}   &---&The /di/ syllable&---&\i\ with a chin.\\
\texttt{syl\_do.mf}   &---&The /do/ syllable&---&c.\\
\texttt{syl\_empty.mf}&---&A placeholder.\\
\end{tabular}
\end{quote}

Let's build this up in reverse direction to save the ligatures for last.
Again, it's probably not the correct way to do it, but it does let us keep a little suspense.

\subsection{\texttt{syl\_empty.mf}}
...
\subsection{\texttt{syl\_do.mf}}

A unique character but the \MF\ code should be unsurprising.
...
\subsection{\texttt{syl\_di.mf}}

Manually creating an accented character.
We can actually stick to only \MF\ when working this out.
...
This use of the \texttt{ligtable} command is new, but it matches much better with its name.
In this case we are actually defining ligatures.
So how does this work?
You type the word \texttt{Bia} into your \TeX\ document because $\ldots$ you want to.
\TeX\ asks the \texttt{tfm} file for the size of the box for the letter \emph{B}.
The \texttt{tfm} file says that the width is the width provided by \texttt{syl\_ba.mf}.
Then \TeX\ asks the \texttt{tfm} file for the size of the box for the letter \texttt{i}.
The \texttt{tfm} file has a ligature table for \emph{B} and also confirms that there is an entry
in that ligtable for \emph{i} and that the glyph to look if is found as character code \texttt{hex"0082"}
that says that "Bi" means the glyph defined by \texttt{syl\_bi.mf}.
So the \emph{B} is deleted (along with its box) and a new box is created for \texttt{hex"0082"}.
Then \TeX\ asks the \texttt{tfm} file for the size of the box for the letter \texttt{a}.
The character is currently \texttt{hex"0082"}, so the ligature table is checked for a
\texttt{hex"0082"} (since we no longer have the \emph{B}) and finds that there is a ligtable for
that character.
This entry is checked for a value of \emph{a} and there is one, so the \texttt{hex"0082"} (along
with its box) are removed and a new character is added for \texttt{hex"0081"}.
After that there is no ligature table entry for \texttt{hex"0081"} so \TeX\ keeps moving along
excited that more ligature work may be coming again.

You have probably noticed that in addition to having an empty a character, I also define a
charcter for a b all by itself even though my invented writing system does not have a way to write
only a b.
The empty character is to keep \TeX\ from complaining, the ``ba'' standing in for ``b'' is to
stop \MF\ from complaining.
In fact, if we are missing this character from the font, \MF\ will refuse to build the font
because it is listed in the \texttt{ligtable}.

Let's test this out.
First we build the font:
...
I mentioned at the beginning of this lesson that you can combine as many letters as you want,
\emph{assuming you have enough space}.
\MF\ still limits you to two-hundred-fifty-six glyphs so if your allowable syllables includes
\emph{springs} you will probably run out of available glyph locations.

\subsection{Multiple fonts}

One way to address that is to have multiple fonts.
The downside to that is that you can't just type it.
Instead of typing \emph{springs} you have to type \verb|{\fontpartc\char"4B}|, which can be made
to look nice when printed but is really painful to handle properly and requires extremely tedious
proofreading.
If you really want this many letters, then you probably want to continue with the lessons.

But, wait a second $\ldots$

Why am I writing \MF\ code like \verb|char_code := hex "0040";|?
Why not just \verb|char_code| \verb|:= hex "40";|?

Try out using numbers like \texttt{hex "0123"} and \texttt{hex "1234"} and you will find that
\MF\ turns these into 35 (hex ``23'') and 52 (hex ``34'').
So using four digits provides no harm, but I want to be ready for the future.

\subsection{Advanced ligatures}

We have only used the most basic of ligatures available.
\MF\ actually allows for quite a bit of ligature replacements.

\begin{center}
\begin{tabular}{lr@{}c@{}llll}
\texttt{a: b}&          &\texttt{=:}&            &\texttt{c;}&``ab'' becomes ``c''  &process c next\\
\texttt{a: b}&\texttt{|}&\texttt{=:}&            &\texttt{c;}&``ab'' becomes ``ac'' &process a next\\
\texttt{a: b}&\texttt{|}&\texttt{=:}&\texttt{>}  &\texttt{c;}&``ab'' becomes ''ac'' &process c next\\
\texttt{a: b}&          &\texttt{=:}&\texttt{|}  &\texttt{c;}&``ab'' becomes ``cb'' &process c next\\
\texttt{a: b}&          &\texttt{=:}&\texttt{|>} &\texttt{c;}&``ab'' becomes ``cb'' &process b next\\
\texttt{a: b}&\texttt{|}&\texttt{=:}&\texttt{|}  &\texttt{c;}&``ab'' becomes ``acb''&process a next\\
\texttt{a: b}&\texttt{|}&\texttt{=:}&\texttt{|>} &\texttt{c;}&``ab'' becomes ``acb''&process c next\\
\texttt{a: b}&\texttt{|}&\texttt{=:}&\texttt{|>>}&\texttt{c;}&``ab'' becomes ``acb''&process b next\\
\end{tabular}
\end{center}

What does ``process $n$'' mean?
That first one is the one we used before for the syllable of \texttt{bia}.
When we typed ``bi'' it became ``syl\_bi'' and then \TeX\ looked at the ``syl\_bi'' table to see
that ``syl\_bia'' also needed to be replaced.

For the next one we could have ``ab'' turn into ``ac'', and then \TeX\ would check the ``a''
table again in order to see if ``ac'' should be replaced with something new.

For the third one we could also have ``ab'' turn into ``ac'', but then \TeX\ would consider the
``a'' fully accepted and would now start looking at the ``c'' table to see what to replace next.

This is also why the last type has three different versions.
Should \TeX\ reprocess the entire thing, consider ``a'' fully accepted, or consider ``ab'' fully
accepted?

These rules can be fun to play with, but I must admit that our use of the \texttt{ligtable}
command has come to an end.
Really I just wanted you to become comfortable with the idea of kerning and ligatures.
They way we will actually do this for final fonts is a bit different.
...
anyone ever bothered to make any fonts.
Just find the correct parameters, feed \MF\ Computer Modern, and you have a new font.
made any fonts.
...
(and display size, and magnification, and style, and $\ldots$), suddenly fonts aren't so nice.

Then vector fonts were created!
Suddenly, you didn't need separate fonts for different display sizes.\footnote{In good typography
you still need different fonts for different display sizes, but many people assumed you did not.}
You now only needed one font for every device!
And \MF\ was be perfect for this because it is already describing fonts as lines instead of as
